...
- MUST cite the literal shell command and a verbatim slice of its captured output (a short tail, ~10 lines max) in every `tool_call` event's payload; MUST NOT paraphrase, summarize, or fabricate git or filesystem state inside event payloads.
...
9. **Cleanup.** When `{delete_after_merge}` resolves to `true`, remove every wrapped worktree via `git worktree remove` (use `--force` only when the worktree is dirty or locked) and delete every wrapped branch via `git branch -d` (or `-D` when the branch is sample+revert net-no-op). Also remove the per-category `<category>-leftovers` aggregate worktrees from step 4 and the `wrap-up-aggregate` worktree from step 7, plus their branches — their content is merged, so they are safe to delete. Emit one `worktree.deleted` and one `branch.deleted` event per cleanup.
10. **Save trajectory and learnings (optional).** When `{save_trajectory}` is `true` (explicit, or interactive confirmation when `-i` is set), write the session trajectory — the thread's chronological timeline of messages, decisions, and tool activity, per the `trajectory-snapshot` skill when available — to `{trajectory_path}` (distinct from the wrap-up event log at `{event_log_path}`), and a markdown summary of decisions made, surprises encountered, and patterns discovered to `{learnings_path}`. Create parent directories as needed. Emit `trajectory.saved` and `learnings.saved` events with absolute paths and line counts.
