...
3. Plan replication: validate first per the parameter constraints in Success Criteria (aborting with an explanatory error and no partial reports on failure), then schedule `{num_experiments}` independent analysis runs, dispatching up to `{parallel}` at a time using `{model}`. When `{num_experiments} = 1`, run a single inline analysis without scheduling; the dispatch sentence applies only when `{num_experiments} > 1`.
