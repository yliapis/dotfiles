...
The reply shape depends on `{update_mode}`: `plan` (default) returns the rendered prompt as the entire reply, inside a single fenced ` ```markdown ` block, with no extra commentary before or after; `agent` writes the rendered prompt (and any propagated referenced-prompt updates) to disk and replies with exactly one line — `Wrote N file(s).` — and no fenced block; `dry-run` returns a single fenced ` ```markdown ` block previewing the intended writes (manifest, diffs, full bodies of new files) without touching the filesystem; `confirm` returns the fenced block and, when `--interactive`/`-i` is set, asks for y/n confirmation before writing (without the flag, it behaves like `plan`). A clarifying question may always precede the reply.
...
- `{save_path}` — optional path at which to persist the generated prompt as a slash command. Inferred from `{input}` context when the user expresses save intent (e.g., "save this", "make it a slash command", "create a `/<name>` command", or a literal path). When intent is detected but no path is given, default to `.cursor/commands/<kebab-name>.md` where `<kebab-name>` is derived from the H1 title. When no save intent is detected, leave unset and write nothing. Drives a write only when `{update_mode}` resolves to `agent` (or `confirm` answered yes); in `plan`, `dry-run`, and unconfirmed/declined `confirm`, the path is resolved for reporting purposes only.
- `{worktree}` — optional name of a git worktree to write the persisted prompt file into; only relevant when `{save_path}` is set, `{n}` is `1`, and `{update_mode}` resolves to `agent` (or `confirm` answered yes). Default: empty. When empty, the workflow derives 2–4 kebab-case worktree-name suggestions from the H1 title; in `interactive` mode the user picks one (or declines, in which case the file is written into the current working tree), and in `non-interactive` mode the first suggestion is used.
...
- [ ] In CREATE or REFINE mode with `{n} = 1` and `{update_mode}` = `plan` (or `confirm` without `--interactive`/`-i`, or `confirm` answered no), the reply contains exactly one fenced ```markdown block whose body matches the Output Format.
- [ ] In CREATE or REFINE mode with `{n} = 1` and `{update_mode}` = `agent`, the reply is exactly the line `Wrote N file(s).` (where `N` is the number of files written) with no fenced block and no manifest.
...
- [ ] In CREATE or REFINE mode with `{n} > 1` and `{update_mode}` = `agent`, the reply is a manifest of the per-variant file paths (one per line) and contains no top-level fenced markdown block.
- [ ] In CREATE or REFINE mode with `{n} > 1` and `{update_mode}` ∈ {`plan`, `dry-run`, unconfirmed `confirm`, declined `confirm`}, the reply is a sequence of `{n}` numbered fenced ```markdown blocks (`## Variant 1` ... `## Variant N`) and no variant file is copied into the calling worktree.
...
- [ ] If save intent was detected, `{n}` is `1`, and `{update_mode}` is `agent` or `confirm` (interactive, answered yes), the file at `{save_path}` exists and contains the rendered prompt body verbatim. In every other `{update_mode}` value, the file at `{save_path}` is neither created nor modified by this invocation.
- [ ] When `{update_mode}` ∈ {`plan`, `dry-run`} or `{update_mode}` = `confirm` with `--interactive`/`-i` absent or the user declined, no file at `{save_path}` and no referenced-prompt file is created or modified by this invocation.
- [ ] When `{worktree}` resolves to a non-empty name, `{n}` is `1`, and a write occurs (`{update_mode}` = `agent`, or `confirm` answered yes), the persisted file is written inside that git worktree (created via `git worktree add` if absent) and no files are written into the original working tree.
...
- [ ] Parameters `{n}`, `{k}`, and `{update_mode}` are validated before any worktree is created: `{n}` is an integer `>= 1`, `{k}` is an integer in `1..{n}`, and `{update_mode}` is one of `plan`, `agent`, `dry-run`, `confirm`. Validation failure aborts with an explanatory error and no filesystem side effects.
...
- [ ] When `{n} > 1` and `{update_mode}` = `agent` (or `confirm` interactive answered yes), after all agents complete, `{n}` variant files exist in the calling worktree at `<save_path>` with `-<i>` appended before the file extension (1-based); no worktree branch is merged.
...
- MUST NOT write any file (primary `{save_path}` or referenced prompts) when `{update_mode}` is `plan` or `dry-run`, regardless of `{save_path}` resolution.
...
4. **Validate parameters.** Confirm `{n}` is an integer `>= 1` and `{k}` is an integer in `1..{n}` (default `{k} = {n}`). Confirm `{update_mode}` ∈ `{plan, agent, dry-run, confirm}`. When `{update_mode}` = `agent`, also confirm that save intent is detectable from `{input}` (explicit `{save_path}` or save-intent phrasing); if not, abort with an explanatory error naming the missing slot. Validation failure on any check aborts with an explanatory error and no filesystem side effects.
...
9. **Emit reply and persist per `{update_mode}` (when `{n} = 1`).** First compute the intended write set: `{save_path}` (or, per the `{input}` carve-out, the referenced-prompt path that becomes the sole authoritative target) for the primary, plus every edited entry in `{referenced_prompts}` for which the user asked to persist or implied command-file updates ("refine `@…`/command"). Then branch on `{update_mode}`:
   - **`plan`** (default): emit the rendered prompt body inside a single fenced ```markdown block as the entire reply. Write nothing; `{save_path}` and `{worktree}` are ignored as write triggers.
...
10. **Fan out and aggregate (when `{n} > 1`).** When `{n} > 1`, the calling agent skips steps 5 through 9 (single-agent gather / rules / propagation / render / persist) and instead:
   - Resolve `{save_path}` (explicit or inferred). When `{update_mode}` = `agent` (or `confirm` interactive), abort with an explanatory error if `{save_path}` cannot be resolved, since variant files require a destination. `{update_mode}` ∈ {`plan`, `dry-run`} and non-interactive `confirm` have no `{save_path}` precondition.
   ...
   - **`{update_mode} = confirm` with `--interactive`/`-i`**: spawn each agent with `update-mode=plan` and collect their fenced blocks. The calling agent emits the aggregated reply (numbered as above), then calls `AskQuestion` once asking whether to persist all `{n}` variants. On yes, re-dispatch each agent with `update-mode=agent` writing to its worktree, copy variant files back into the calling worktree, and append a per-variant write status to the reply. On no, write nothing further.
...
Mention save intent in your input (e.g. "save this", "make it `/foo`") and a `{save_path}` is resolved to `.cursor/commands/<kebab-name>.md` by default. The actual write only happens when `update-mode=agent` (or `update-mode=confirm` answered yes); the default `update-mode=plan` shows the block without writing. Pass `worktree=<name>` to persist the file inside a named git worktree; leave it empty to pick from 2–4 suggested kebab-case names derived from the title (auto-picked when interactive mode is off).
...
**Update mode**
Pass `update-mode=<value>` to control what happens after rendering. Default: `plan`.
- `plan` (default) — show the rendered prompt as a fenced markdown block; write nothing. Save intent alone no longer triggers a write; you must request `agent` mode to persist.
- `agent` — write the rendered prompt (and any propagated referenced-prompt updates) to disk silently; the reply is a single `Wrote N file(s).` line. Requires save intent or an explicit `{save_path}`; aborts otherwise.
...
- `confirm` — show the block, then ask y/n (interactive only); on yes, behave like `agent`. Without `-i`/`--interactive`, falls back to `plan`.
...
Pass `n=<int>` to fan out into `n` parallel meta-prompt agents, each in its own worktree, all running the same `/meta-prompt` invocation. Pass `k=<int>` to cap the number of agents active concurrently (default `k = n`). Aggregation depends on `update-mode`: in `agent` (and `confirm` answered yes), every agent writes its rendered prompt to a variant file (`<save_path>` with `-<i>` appended before the extension) inside its own worktree and the calling agent copies every variant file back into the calling worktree — this requires save intent (explicit or inferable); without it the invocation aborts. In `plan` and `dry-run` (and unconfirmed/declined `confirm`), each agent's rendered block is collected and the calling agent emits `n` numbered fenced blocks inline, with no save-intent requirement. No branch merge is ever performed.
...
- `/meta-prompt update-mode=agent save as /pr-review: review a PR for security issues`
