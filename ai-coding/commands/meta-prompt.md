...
10. **Fan out and aggregate (when `{n} > 1`).** When `{n} > 1`, the calling agent skips steps 5 through 9 (single-agent gather / rules / propagation / render / persist) and instead:
   ...
   - **`{update_mode} = confirm` with `--interactive`/`-i`**: spawn each agent with `update-mode=plan` and collect their fenced blocks. The calling agent emits the aggregated reply (numbered as above), then calls `AskQuestion` once asking whether to persist all `{n}` variants. On yes, the calling agent writes each already-collected variant body verbatim to `<save_path>` with `-<i>` appended before the extension in the calling worktree (no re-dispatch — the approved content is already in hand and re-running agents would persist different text than the user approved), and appends a per-variant write status to the reply. On no, write nothing further.
