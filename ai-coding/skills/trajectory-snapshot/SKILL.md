...
Complements the `save-session-state` command: that captures resumable *state*; this skill captures the *trajectory* (what happened, in order). Filename and default location follow the dump convention owned by the `file-dump` skill (`{description}-{model}-{date}-{session}` under `.ai-coding-artifacts/dumps/`).

## Parameters
- `{filename_format}` — output path template; optional, default: `.ai-coding-artifacts/dumps/{coding_tool}-snapshot-{model}-{datetime_timestamp}-{session}.md`, resolved relative to the current workspace root (MAY include directory components or an absolute path). Recognized tokens: `{coding_tool}` (resolved tool slug), `{model}` (slug of the model powering the session, normalized to lowercase with spaces and slashes replaced by hyphens, dots kept; `unknown-model` when undeterminable), `{datetime_timestamp}` (filesystem-safe UTC from `date -u +%Y-%m-%dT%H-%M-%SZ`, e.g. `2026-07-04T07-25-30Z`), and `{session}` (first 8 characters of the session id; `unknown` when undeterminable). The default renders like `.ai-coding-artifacts/dumps/cursor-snapshot-claude-4.5-opus-2026-07-04T07-25-30Z-a1b2c3d4.md`. When `{granularity}` is `native`, the `.md` extension is replaced by the native transcript's extension (e.g. `.jsonl`).
...
- [ ] The path is rendered from `{filename_format}` with `{coding_tool}`, `{model}`, `{datetime_timestamp}`, and `{session}` substituted; undeterminable `{model}` / `{session}` render as `unknown-model` / `unknown`; no unresolved `{…}` tokens remain in the final path.
...
2. Detect `{coding_tool}` when not provided: `cursor` if a project transcript dir `~/.cursor/projects/<workspace-slug>/agent-transcripts/` exists for this workspace, `claude-code` if `~/.claude/projects/<munged-cwd>/` exists, else `agent`. Detect `{model}` from the session's own metadata or system context and `{session}` from the transcript location; fall back to `unknown-model` / `unknown` rather than guessing.
3. Render the output path from `{filename_format}` using `{coding_tool}`, `{model}`, `{session}`, and `date -u +%Y-%m-%dT%H-%M-%SZ`; for `native`, swap the `.md` extension for the native transcript's extension.
