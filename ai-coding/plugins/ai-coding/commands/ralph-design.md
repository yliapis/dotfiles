...
## Task
Run an interactive ralph-loop that walks the design walkthrough enumerated by `{design_skill}` one step at a time to produce that skill's declared deliverable for `{domain}`, with optional per-round fan-out via `/worktree-task-agent` to explore variants in parallel.
...
- `{fanout_default_n}` — default number of sibling variants spawned via `/worktree-task-agent` when the user picks `fork` at a round and supplies no explicit count; optional, default: `4`. MUST be an integer `>= 2`.
- `{agent_model}` — model identifier passed through to `/worktree-task-agent` for fan-out agents; optional, default: the parent agent's model. Broadcast to every spawned agent within a `fork`.
...
- [ ] Every round produces a snapshot of the working artifact tagged by round index and step slug. When `{persistence} = codebase`, snapshots are written to `{artifact_path}.snapshots/<round>-<step-slug>.md`; when `{persistence} = chat`, snapshots are held in memory only.
- [ ] In `interactive` mode, every round prompts the user with the choices `accept` / `refine` / `fork` / `back` / `done`. `accept` applies the draft, snapshots, and advances. `refine` takes the user's feedback and redrafts the same step without advancing. `fork` dispatches `/worktree-task-agent`, presents the variants' artifact files, and applies the user's pick before advancing. `back` rewinds to a user-named earlier step's snapshot and resumes forward step-by-step from the next step. `done` exits the loop with the current artifact.
...
- MUST NOT recurse: fan-out agents dispatched through `/worktree-task-agent` MUST run with `parallelism = 1` and MUST NOT invoke `/ralph-design` or further fan out.
...
- MUST NOT push, open PRs, merge branches, delete worktrees, create tags, or rebase pre-existing branches; the iterator's worktree and its branch are left in place for the user to review and merge separately, and fan-out worktrees are left to `/worktree-task-agent`'s own policy.
- Scope: walking one design skill's walkthrough for one `{domain}` to produce that skill's declared deliverable, optionally exploring per-round variants through `/worktree-task-agent`. Out of scope: composing multiple design skills in one loop (use `/designer` for trait composition), generating downstream code or types, publishing the artifact to a registry, and merging the iterator's worktree branch back into `{base_branch}`.
...
- `Terminal state`: `user-done` / `all-steps-complete` / `validation-pass` / `max-rounds-hit` / `iteration-cap-hit` / `plan-declined` / `validation-error`.
...
- For `forked` rounds: sibling worktrees created by `/worktree-task-agent` are left in place per its own merge policy; their diffs are addressable via that command's own report.
