...
1. **Parse flags and parallelism args.** Extract `--interactive` / `-i` from `{input}` and remove it; record interactive mode as on iff the flag is present (default off). Extract `n=<int>` and `k=<int>` tokens (whitespace-delimited) and remove them; record `{n}` (default `1`) and `{k}` (default `{n}`).

2. **Detect mode** from the remaining `{input}`:
...
3. **Validate parallelism parameters.** Confirm `{n}` is an integer `>= 1` and `{k}` is an integer in `1..{n}` (default `{k} = {n}`). Abort with an explanatory error and no filesystem side effects on validation failure.

4. **Gather missing slots.** Decide what the input pins down vs. what you need:
...
6. **Render** the prompt using the Output Format below. Omit any section that has no real content.

7. **Persist if requested (when `{n} = 1`).** If `{save_path}` is set (explicit or inferred), write the rendered prompt body — the contents of the fenced block, without the outer fence — to that path, creating parent directories if needed. When `{worktree}` resolves to a non-empty name, perform the write inside that git worktree, creating it via `git worktree add` off the current branch if it does not already exist; otherwise write into the current working tree. When no save intent is detected, write nothing.

8. **Fan out and aggregate (when `{n} > 1`).** When `{n} > 1`, the calling agent skips steps 4–7 above and instead:
