# Create Tickets Contract

Loaded by the `operating-tickets` `create` motion to turn one source of
proposed work into local ticket files and a `WORKLIST.md`. This motion
supersedes `ticket-breakdown`.

## Applicability

Do not use it to produce the source analysis, operate GitHub/Jira/Linear, execute
ticket work, or revise an existing ticket. Those requests route to the
`operating-tickets` execute or update motion.
...
`fingerprint`: the update motion recomputes it after an explicit definition
change, while the execute motion preserves it. Extensions change only through
an explicit merge patch and remain excluded from the fingerprint.
...
exact criterion text and current fingerprint. The execute motion produces and
runs fresh evidence; the update motion may adopt explicit caller-supplied
evidence but labels it `caller-supplied; not executed`.
...
Generated by operating-tickets/create from `<source-identifier>`.
```
