...
- **Missing shape parameters become proposals.** A `plan` round proposes a value for any shape parameter the invocation left open and the skill has no default for — `{num_agents}`, `{topology}`, `{num_partitions}`, `{model_mix}` — from the Sizing Heuristics, the Topology Catalog, and the Pattern Catalog, rather than failing validation for it. A parameter with a documented default keeps that default and is not a proposal. Nothing executable is ever proposed: `{test_command}` and `{aggregator}` stay unset until the user supplies them, and the selection modes that need them stay skipped. Proposals are labeled as such in `### Plan Revision` and bind nothing until the user verifies the design. This is why `plan` is the one mode whose loop owns validation: the missing shape parameter is often the thing the user came to decide.
...
- Every slot node carries its own resolved model, which keeps a `per-agent` mix readable and gives the fold rule a uniform label to work on; group nodes carry only their own label. Under `plan`, a label whose value the round proposed rather than took from the user ends in `?`, so the diagram says which numbers are still open.
