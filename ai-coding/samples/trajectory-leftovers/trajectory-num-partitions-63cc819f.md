...
- `{num_partitions}` was added to express grouping of agents/models: a partition is a slice of `{parallelism}/{num_partitions}` consecutive agents that share an `{agent_model}` (so `{num_partitions}` must divide `{parallelism}` evenly). `{agent_model}` accepts three shapes — scalar (e.g., `"sonnet"`, broadcast to all `{parallelism}` agents); list-by-agent (length `{parallelism}`, one entry per agent — e.g., 8 entries when `{parallelism}=8`); list-by-partition (length `{num_partitions}`, one entry per partition broadcast to its `{parallelism}/{num_partitions}` agents — e.g., 4 entries when `{num_partitions}=4` and `{parallelism}=8`).
