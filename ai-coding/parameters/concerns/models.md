...
- `{model_mix}` (agent-swarm) — the model-assignment *shape* for the swarm:
  ...
  `agent_model` at dispatch time. The swarm expands the mix into a per-slot
  assignment over its global slots, then hands each dispatch only the slots it
  owns, so the child's `len(agent_model) == parallelism` check holds whether
  the topology dispatches once (`parallel`, `partitioned`:
