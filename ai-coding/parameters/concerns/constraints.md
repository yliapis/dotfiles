...
### `test_command`
- **Aliases:** `verify_command` (address-worklist-commit-loop)
...
- **Meaning:** A per-candidate verifier run inside the candidate's workspace
  ...
  item's edits are staged but before the commit (address-worklist-commit-loop,
  where a failure suppresses the commit, hard-resets the tree, and triggers
  `{on_failure}`).
- **Validation:** agent-swarm requires it when any of `auto-best`, `vote`,
  `hybrid`, `tournament` is in `{selection_modes}`.
  address-worklist-commit-loop exports `WORKLIST_ITEM_*` variables into its
  environment.
