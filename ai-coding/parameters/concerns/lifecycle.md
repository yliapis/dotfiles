...
- **Aliases:** `update_mode` (meta-prompt), `persistence` (designer,
  design-skill, ticket-crud compatibility),
  `dry_run` (ticket-crud, ticket-execute,
...
- **Meaning:** Whether the run mutates the filesystem or only shows what it
  ...
  | ticket-crud | `{dry_run}` | `true`, `false` (default) | create validates and renders the complete set in memory and writes no lock, staging directory, or artifact; update renders the prospective transaction and plan digest without lock, journal, file mutation, staging, or commit |
...
- **Validation:** the design skills reject `artifact_path` / `use_worktree`
  ...
  save target; ticket-crud accepts only compatibility
  `{persistence}=files` and `{emit_worklist}=true`.
- **Used by:** meta-prompt, designer, design-skill,
  address-worklist-commit-loop, save-session-state, ticket-crud,
  ticket-execute
...
- **Deterministic managed-set reuse** — ticket-crud never overwrites or adds
...
- `{on_existing}` (ticket-crud create) — `reuse` (default) | `error` |
  `new-set`; controls deterministic same-lineage reuse and collision behavior.
- `{emit_worklist}` (ticket-crud compatibility) — only explicit `true` is
  accepted; native sets always include `WORKLIST.md`.
- `{projection}` (ticket-crud update) — `require` (default) | `repair`; repair
  is limited to fields deterministically projected from authoritative tickets.
- `{commit}` (ticket-crud update) — `none` (default) | `audit`; audit creates
  one local commit containing or hashing the managed-state transaction.
- `{operation}` (ticket-crud) — `create` | `update`; selects which half of the
