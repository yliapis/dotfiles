...
- **Aliases:** `-i` / `--interactive` (critique, meta-prompt, wrap-up,
  save-session-state, soft-shutdown, ticket-crud),
...
- **Meaning:** Opt-in to clarifying questions and confirmations. When absent,
  ...
  | ticket-crud | presents the validated ticket plan for approval before atomic publication of a created set (otherwise ambiguities go to Open Questions and the plan is not gated) |
...
- **Used by:** critique, meta-prompt, wrap-up, save-session-state,
  soft-shutdown, prompt-template-library, ticket-crud

### `approval_mode`
- **Aliases:** `mode` (ticket-execute, ticket-crud update,
  address-worklist-commit-loop compatibility command, agent-swarm),
  `approval` (designer)
- **Applies to:** command, skill
- **Type:** ticket-execute, the compatibility command, and designer:
  `interactive` | `non-interactive` | `force-approve-all`; ticket-crud:
...
- **Used by:** ticket-execute, ticket-crud,
