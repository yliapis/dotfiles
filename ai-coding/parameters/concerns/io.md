...
- `{coding_tool}` (trajectory-snapshot) — tool slug substituted into
  ...
  `agent`.
- `{granularity}` (ticket-crud create) — ticket splitting policy: closed enum
...
- `{id_prefix}` (ticket-crud create) — prefix for rendered ticket ids
  (`{id_prefix}-NNN`, zero-padded). Default `TKT`. Validation:
  `^[A-Z][A-Z0-9]{1,9}$`.
- `{output_dir}` (ticket-crud create) — directory the ticket files and
