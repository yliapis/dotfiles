...
- `{coding_tool}` (trajectory-snapshot) — tool slug substituted into
  ...
  `agent`.
- `{granularity}` (operating-tickets create) — ticket splitting policy: closed enum
...
- `{id_prefix}` (operating-tickets create) — prefix for rendered ticket ids
  (`{id_prefix}-NNN`, zero-padded). Default `TKT`. Validation:
  `^[A-Z][A-Z0-9]{1,9}$`.
- `{output_dir}` (operating-tickets create) — directory the ticket files and
