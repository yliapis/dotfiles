...
`.cursor/`, `.claude/`, `.opencode/`, `.agents/`, and `.codex/` each carry
flattened `commands/`, `skills/`, and `agents/` mirrors of
`ai-coding/plugins/*/{commands,skills,agents}`, which is what those tools
read when this repository is the workspace.
`ai-coding/plugins/` is the only place to edit; every one of the fifteen
mirrors is generated by
...
from `.agents/skills` and user skills from `$HOME/.agents/skills`.
`.codex/` is the OpenAI Codex home. Official repo skills stay in
`.agents/skills`; `$CODEX_HOME/skills` is a deprecated compatibility
location. Codex has no plugin-marketplace equivalent of `.cursor-plugin`
or `.claude-plugin`. The trees still emit `commands/` and `agents/` so the
folders match the other AI capability trees.
