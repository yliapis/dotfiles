...
Code, and OpenCode read when this repository is the workspace. Codex reads a
tool-neutral [`.agents/skills/`](.agents/) tree — skills only, because it has no
repo-root path for commands and reads subagents only from home as TOML; see
[`.agents/README.md`](.agents/README.md) for the research trail.
`ai-coding/plugins/` is the only place to edit; every one of the ten mirrors is
generated by
...
The same script generates an eleventh mirror, `.claude/hooks/` from
[`ai-coding/hooks/`](ai-coding/hooks) — see Cloud Agents below.

Codex also has a marketplace catalog at
[`.agents/plugins/marketplace.json`](.agents/plugins/marketplace.json) — parallel
to `.cursor-plugin/` and `.claude-plugin/`, with a `.codex-plugin/plugin.json` in
each plugin. `make sync-codex` copies the skills to `~/.agents/skills/` and the
marketplace bundle to `~/.agents/plugins/` (commands and agents are skipped).
