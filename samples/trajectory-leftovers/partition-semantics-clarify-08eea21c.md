...
- `{num_partitions}` was added to express grouping of agents/models: a partition is a contiguous slice of the `{parallelism}` agents that share one `{agent_model}`, so each partition contains `{parallelism} / {num_partitions}` agents (assuming even division, still open). Three supported `{agent_model}` shapes: scalar (e.g., `"sonnet-4"`) broadcasts to all `{parallelism}` agents; list-by-agent (length = `{parallelism}`) assigns one model per agent positionally; list-by-partition (length = `{num_partitions}`) assigns one model per partition, broadcast to the agents inside each. How non-divisible `{parallelism}` / `{num_partitions}` resolves is still under design.
