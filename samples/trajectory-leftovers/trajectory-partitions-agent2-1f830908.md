...
- `{num_partitions}` was added to express grouping of agents/models: a partition is a contiguous block of `{parallelism} / {num_partitions}` agents that share a model assignment (so `{num_partitions}=1` means all `{parallelism}` agents share one model, while `{num_partitions}={parallelism}` means one model per agent). An iterable `{agent_model}` must therefore be either a full iteration across all agents (length = `{parallelism}`) or a correctly-sized slice across partitions (length = `{num_partitions}`, broadcast within each partition). Concrete shapes for `{parallelism}=8, {num_partitions}=2` (4 agents per partition): scalar — `"gpt-5"` broadcasts to all 8 agents; list-by-agent — `["gpt-5", "gpt-5", "gpt-5", "gpt-5", "claude", "claude", "claude", "claude"]` (length 8 = `{parallelism}`) maps one model per agent; list-by-partition — `["gpt-5", "claude"]` (length 2 = `{num_partitions}`) assigns one model per partition.
