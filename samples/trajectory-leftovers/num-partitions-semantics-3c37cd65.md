...
- `{num_partitions}` was added to express grouping of agents/models. A partition is a contiguous slice of agents that share a model assignment, with `{parallelism} / {num_partitions}` agents per partition (so `{num_partitions}` must divide `{parallelism}` evenly). Three `{agent_model}` shapes are supported: scalar — one identifier broadcast to all `{parallelism}` agents (e.g. `"sonnet"`); list-by-agent of length `{parallelism}` — one entry per agent (e.g. `["sonnet", "sonnet", "opus", "opus"]` for `{parallelism}=4`); list-by-partition of length `{num_partitions}` — one entry per partition, broadcast within it (e.g. `["sonnet", "opus"]` for `{num_partitions}=2`, `{parallelism}=4`, giving agents 1-2 sonnet and 3-4 opus).
