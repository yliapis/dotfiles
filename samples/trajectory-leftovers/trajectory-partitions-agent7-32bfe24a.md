...
- `{num_partitions}` was added to express grouping of agents/models: a partition is a group of `{parallelism} / {num_partitions}` sibling agents (and their worktrees) that share one `{agent_model}` slot. Three supported `{agent_model}` shapes (illustrated for `{parallelism} = 4`, `{num_partitions} = 2`): scalar (e.g. `"sonnet-4"`, broadcast to all 4 agents); list of length `{parallelism}` (e.g. `["sonnet-4", "opus-4", "sonnet-4", "opus-4"]`, one model per agent positionally); list of length `{num_partitions}` (e.g. `["sonnet-4", "opus-4"]`, one model per partition, broadcast to the 2 agents within). Open: the divisibility rule when `{parallelism}` does not divide evenly by `{num_partitions}`.
